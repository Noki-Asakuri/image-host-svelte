<script lang="ts">
    ...
    export let createKey: ({
        author,
        password,
    }: InferMutationInput<"key.create">) => void;
</script>
...
>
    ...
    <div class="flex gap-2">
        <label for="author" class="flex items-center justify-center gap-2">
            Author:
        </label>
        <input
            type="text"
    ...
    <div class="flex gap-2">
        <label for="password" class="flex items-center justify-center gap-2">
            Password:
        </label>
        <input
            type="password"
    ...
    <button
        class="w-full h-10 py-2 bg-slate-700 rounded-2xl drop-shadow-lg"
        type="submit"
    >
        {#if !isLoading}
            Create
